 ...
 * Run: pnpm --filter @workspace/db exec tsx src/scripts/restore-drill.ts
 */
...
import { assertSafeTestDatabaseUrl } from "../env";
...
async function pgliteDrill() {
  ...
  const tables = await source.query(`
    ...
    ORDER BY table_name
  `);
  const names = (tables.rows || []).map((r) => String((r as { table_name: string }).table_name));
  ...
  for (const t of CRITICAL_TABLES) {
    ...
    counts[t] = Number((c.rows?.[0] as { n: number } | undefined)?.n ?? 0);
  }
  writeFileSync(
    dumpPath,
  ...
  const restored = await target.query(`
    ...
    WHERE table_schema = 'public' AND table_type = 'BASE TABLE'
  `);
  const restoredNames = (restored.rows || []).map((r) => String((r as { table_name: string }).table_name));
  for (const t of CRITICAL_TABLES) {
  ...
  await source.close();
  await target.close();
  return { mode: "PGLITE_LOGICAL", dumpPath, counts, status: "PASS" as const };
}

async function postgresDumpOnly(url: string) {
  assertSafeTestDatabaseUrl(url);
  if (!hasPgDump()) {
    return { mode: "POSTGRES", status: "PENDING" as const, reason: "pg_dump not installed" };
  }
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const dumpFile = path.join(outDir, `pg-${stamp}.dump`);
  const dump = spawnSync("pg_dump", [url, "--format=custom", `--file=${dumpFile}`], { encoding: "utf8" });
  if (dump.status !== 0) {
    return { mode: "POSTGRES", status: "FAIL" as const, reason: "pg_dump failed" };
  }
  if (!existsSync(dumpFile) || readFileSync(dumpFile).length < 100) {
    return { mode: "POSTGRES", status: "FAIL" as const, reason: "dump artifact empty" };
  }
  return {
    mode: "POSTGRES",
...
async function main() {
  ...
  let result: Record<string, unknown> = await pgliteDrill();
  if (testUrl) {
    result = { ...result, postgres: await postgresDumpOnly(testUrl) };
  }
